...
Capa 5 Sesion: Es donde se gestionan las sesiones que se comunican entre el servidor y el navegador (PDU: datos)

Capa 4 Transporte: Aqui es donde los datos se segmentan mediante TCP (transmission control protocol) y se garantizan que sean entregados sin errores, secuencialmente y de extremo a extremo. tambien es donde el TCP usa los Handsake de 3 vias para establecer la conexion antes de enviar datos(PDU: segmentos)

Capa 3 Red: Se empaquetan los segmentos con la IP(internet protocol) de origen y destino,  mediante el direccionamiento logico y enrutamiento(router lo usa).(PDU:Paquetes)

Capa 2 Enlace: Aqui es donde se garantiza la transmision, libre de errores, entre dos dispositivos conectados a la misma red local, mediante el uso de direcciones MAC (media acces control) aqui es donde se forma la trama y se dirige el trafico.(PDU: tramas)
...
IP (internet protocol): es para identificar y ubicar cualquer dispositivo en una red

TLS (transpor layer security): es el protocolo en cifra la comunicacion en el trafico. Se puede ver si esta activo cuando en una pagina web esta el HTTPS y el candado

TCP (transmission contro protocol):  protocolo que garantiza la entrega correcta de datos.

MAC (media access control): Es el identificador fisico unico de cada tarjeta de red, esta ene l hardware y es un codigo en formato hexadecimal.

Puertos: son numeros del 0 al 65535 que ayudan a identificar hacia donde deben apuntar los servicios dentro de un servidor

ARP (address resolution protocol): su funcion es detectar la direccion MAC cuando solo conocemos la direccion IP
